...
const ActuationSection = () => {
    return (
        <div className="relative h-auto bg-cover bg-center py-12 px-4 sm:px-8 font-sans" style={{ backgroundImage: "url('/fundo.png')" }}>
            ...
            {/* Content */}
            <div className="relative z-10 text-white p-4 sm:p-8 mx-auto">
                {/* Heading */}
                ...
                {/* Statistics Cards */}
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-12 sm:mt-20 md:mt-40 text-start">
                    <div className="p-4 sm:p-6 rounded-lg">
                        <p className="text-3xl sm:text-4xl md:text-5xl font-bold">50%</p>
                        <p className="mt-2 text-sm sm:text-base md:text-xl w-1/2">da população ativa do território está desempregada</p>
                    </div>
                    <div className="p-4 sm:p-6 rounded-lg">
                        <p className="text-3xl sm:text-4xl md:text-5xl font-bold">48%</p>
                        <p className="mt-2 text-sm sm:text-base md:text-xl w-1/2">da população possui apenas ensino fundamental incompleto</p>
                    </div>
                    <div className="p-4 sm:p-6 rounded-lg">
                        <p className="text-3xl sm:text-4xl md:text-5xl font-bold">70%</p>
                        <p className="mt-2 text-sm sm:text-base md:text-xl w-1/2">da população sofre algum nível de insegurança alimentar</p>
                    </div>
                    <div className="p-4 sm:p-6 rounded-lg">
                        <p className="text-3xl sm:text-4xl md:text-5xl font-bold">20%</p>
                        <p className="mt-2 text-sm sm:text-base md:text-xl w-1/2">da população vive em insegurança alimentar grave (situação de fome)</p>
                    </div>
                </div>

                {/* Description */}
                <div className="mt-8 text-start">
                    <p className="text-sm sm:text-base md:text-xl px-6">
                        Crianças e adolescentes vivendo em condições insalubres, de muita violência, sem oportunidades e perspectiva de vida.
                    </p>
                </div>

                {/* Footer */}
                <div className="mt-16 sm:mt-24 md:mt-32 text-xs">
